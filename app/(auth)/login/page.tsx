...
import { Database, Eye, EyeOff, Loader2, Chrome } from "lucide-react";
...
export default function LoginPage() {
  ...
  const [error,        setError]        = useState<string | null>(
    ...
    googleError === "google_no_email" ? "Google account has no email." : null
  );
  ...
  const handleOtp = async (code: string) => {
    ...
    try {
      const res  = await fetch("/api/auth/verify-otp", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: form.email, code, purpose: "login" }),
      });
      const data = await res.json();
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <button type="button" onClick={() => setShowPassword((p) => !p)}
                      className="absolute right-3 top-1/2 -translate-y-1/2 text-[#484f58] hover:text-[#8b949e] transition-colors cursor-pointer">
