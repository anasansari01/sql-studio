...
async function createEcommerceSandbox(pool: Pool) {
  await pool.query(`
    ...
    -- Clear and re-insert sample data
    ...
    INSERT INTO assignment_ecommerce.orders (customer_id, product_id, quantity, total_amount, ordered_at) VALUES
      (1, 1, 1, 1299.99, '2024-01-10 10:00:00'),
      (1, 2, 2, 59.98, '2024-01-12 11:00:00'),
      (2, 3, 1, 49.99, '2024-01-15 09:30:00'),
      (3, 4, 1, 299.99, '2024-01-20 14:00:00'),
      (3, 6, 1, 39.99, '2024-01-22 16:00:00'),
      (4, 1, 1, 1299.99, '2024-02-01 10:00:00'),
      (4, 5, 1, 599.99, '2024-02-05 12:00:00'),
      (5, 7, 2, 69.98, '2024-02-10 15:00:00'),
      (2, 2, 3, 89.97, '2024-02-15 13:00:00'),
      (1, 7, 1, 34.99, '2024-02-20 11:00:00');
  `);
...
async function createHrSandbox(pool: Pool) {
  await pool.query(`
    ...
    INSERT INTO assignment_hr.employees (name, department_id, salary, hire_date, manager_id) VALUES
      ('Sarah Connor', 1, 120000, '2019-03-01', NULL),
      ('John Reese', 1, 95000, '2020-06-15', 1),
      ('Mary Jane', 1, 88000, '2021-01-10', 1),
      ('Peter Parker', 2, 72000, '2020-09-01', NULL),
      ('Tony Stark', 2, 85000, '2019-11-20', 4),
      ('Steve Rogers', 3, 90000, '2018-07-04', NULL),
      ('Natasha Romanova', 3, 82000, '2021-03-15', 6),
      ('Bruce Banner', 4, 68000, '2022-01-05', NULL),
      ('Clint Barton', 5, 95000, '2019-05-20', NULL),
      ('Wanda Maximoff', 1, 105000, '2020-08-01', 1);
  `);
...
async function createSchoolSandbox(pool: Pool) {
  await pool.query(`
    ...
    INSERT INTO assignment_school.students (name, grade_level, enrollment_date) VALUES
      ('Hermione Granger', 3, '2022-09-01'),
      ('Harry Potter', 3, '2022-09-01'),
      ('Ron Weasley', 3, '2022-09-01'),
      ('Luna Lovegood', 2, '2023-01-15'),
      ('Draco Malfoy', 3, '2022-09-01'),
      ('Neville Longbottom', 3, '2022-09-01');

    INSERT INTO assignment_school.courses (name, credits, instructor) VALUES
      ('Database Systems', 4, 'Prof. Dumbledore'),
      ('Algorithms', 3, 'Prof. McGonagall'),
      ('Web Development', 3, 'Prof. Snape'),
      ('Machine Learning', 4, 'Prof. Dumbledore'),
  ...
  await db.insert(assignments).values([
    {
      ...
      description:
        "Query customer orders to find who has spent the most. Practice JOINs and aggregation.",
      difficulty: "easy",
...
Order the results by total_spent descending.`,
    ...
    },
    {
      ...
      description:
        "Use LEFT JOIN or NOT IN to find products with no orders. Classic set-difference pattern.",
      difficulty: "medium",
...
Order by category, then by name.`,
    ...
    },
    {
      ...
      description:
        "Aggregate orders by month. Practice date functions and GROUP BY.",
      difficulty: "medium",
...
Order by year and month ascending.`,
    ...
    },
    {
      ...
      description:
        "Use window functions or subqueries to find the top earner in each department.",
      difficulty: "hard",
...
Order by salary descending.`,
    ...
    },
    {
      ...
      description:
        "Simple GROUP BY with aggregation. Great for beginners learning aggregate functions.",
      difficulty: "easy",
...
Order by avg_salary descending.`,
    ...
    },
    {
      ...
      description:
        "Calculate GPA across multiple enrollments. Multi-table aggregation with HAVING.",
      difficulty: "medium",
