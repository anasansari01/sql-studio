...
export async function validateAnswer(
  ...
  if (!solutionResult.success) {
    ...
    return {
      isCorrect: false,
      feedback:
        "Internal error: the reference solution failed to run. Please contact support.",
    };
  }
  ...
  if (!colsMatch) {
    ...
    if (extra.length > 0)
      feedback += ` Unexpected: ${extra.join(", ")}.`;
  ...
  if (studentRowCount !== expectedRowCount) {
    ...
    if (studentRowCount > expectedRowCount) {
      feedback += " You may be missing a HAVING/WHERE filter or producing duplicates.";
